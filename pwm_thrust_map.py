...
┌─────────────────────────────── ⚠️ 안전 ───────────────────────────────┐
│ 이 스크립트는 모터를 '실제로 회전'시킨다.                                    │
│  - 스러스트 스탠드/모터를 단단히 고정, 회전면(궤적)에 사람·손·물건 금지.        │
│  - 보안경 착용, 물러선 상태에서 실행.                                        │
│  - '시동 해제' 상태에서만 동작(시동 상태면 PX4가 명령을 거부).                   │
│  - GUI의 빨간 STOP은 '즉시 정지'. 브라우저를 닫거나 끊기면 watchdog이 자동 정지. │
│  - 처음엔 낮은 범위(예 B 1000~1200)로 짧게 확인한 뒤 범위를 넓히는 것을 권장.     │
│  - 최후엔 배터리 물리적 분리.                                                │
└──────────────────────────────────────────────────────────────────────┘
